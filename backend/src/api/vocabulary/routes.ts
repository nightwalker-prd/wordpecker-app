...
import { vocabularyServiceAdapter } from './adapter';
...
router.post('/generate-words', openaiRateLimiter, validate(generateWordsSchema), async (req, res) => {
  try {
    ...
    const vocabularyWords = await vocabularyServiceAdapter.generateWords(
      wordCount, 
      difficulty, 
      context, 
      baseLanguage, 
...
router.post('/get-word-details', openaiRateLimiter, validate(getWordDetailsSchema), async (req, res) => {
  try {
    ...
    const { baseLanguage, targetLanguage } = await getUserLanguages(validation.userId);
    const wordInfo = await vocabularyServiceAdapter.getWordDetails(word, context, baseLanguage, targetLanguage);
